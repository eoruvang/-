...
st.markdown("공정 변수와 PID 설정을 실시간으로 조절하고, Google AI Studio Gemini API를 통해 제어 성능을 분석받으세요.")

# -----------------------------
# 사이드바: API 키 및 파라미터 설정
# -----------------------------
st.sidebar.header("🔑 Google AI Studio API 키")
...
# -----------------------------
# Google AI Studio 연동 부분
# -----------------------------
st.divider()
st.subheader("🤖 AI 제어 성능 분석 (Google AI Studio)")

if st.button("Gemini AI로 튜닝 상태 분석하기"):
    if not api_key:
        st.warning("왼쪽 사이드바에 Google AI Studio API 키를 입력해 주세요.")
    else:
        try:
            ...
            위 설정이 적절한지 평가하고, 응답 속도를 개선하거나 오버슈트/잔류편차를 줄이기 위해 Kc, Ti, Td를 어떻게 수정해야 할지 조언해 주세요.
            """
            
            with st.spinner("AI가 결과를 분석 중입니다..."):
